  ...
  useEffect(() => {
    ...
    getCountries.then(response=>{
      const mappedCountries = response.map(country=>({
        ...
        countryLatLng: country.capitalInfo && country.capitalInfo.latlng && country.capitalInfo.latlng.length>0 ? {lon: country?.capitalInfo?.latlng[0], lat: country?.capitalInfo?.latlng[1]} : null,
      }));
